...
# Configuration
INPUT_DATA_DIR = Path("./ucreat_echr_events/arg_vs_non-arg_ucreat_events_echr")
MODEL_PATH = Path("../14 events/DATA5/7 ucreat events/suyamoon_tmp/7 ucreat events remaining/Results/results_arg_vs_non-arg_with_ucreat_events/RoBERTa/best_model")
OUTPUT_DIR = Path("./predictions/arg_vs_non-arg_ucreat_events_echr/RoBERTa")
BATCH_SIZE = 8
...
def predict_on_dataset(model, tokenizer, dataset):
    ...
    def tokenize_function(examples):
        return tokenizer(
            ...
            padding="max_length",  # Changed from "max_length" to ensure consistent tensor sizes
            truncation=True,
            max_length=512,
    ...
    tokenized_dataset = dataset.map(
        tokenize_function,
        batched=True,
        remove_columns=["processed_text"] + columns_to_remove  # Remove processed_text and any label columns
    )
...
def process_single_file(file_path, model, tokenizer):
    ...
    try:
        ...
        df['prob_non_arg'] = probabilities[:, 0]  # Probability for class 0 (Non-Argumentative)
        df['prob_arg'] = probabilities[:, 1]      # Probability for class 1 (Argumentative)
        
        # Map predictions to readable labels
        label_mapping = {0: 'Non-Argumentative', 1: 'Argumentative'}
...
def main():
    ...
    if successful_predictions > 0:
        ...
        print("- predictions: Numerical predictions (0: Non-Argumentative, 1: Argumentative)")
        print("- prediction_label: Human-readable prediction labels")
        print("- prob_non_arg: Probability for Non-Argumentative class")
        print("- prob_arg: Probability for Argumentative class")
