...
MODEL_PATH = "./InCaseLawBERT_best_model"  # Relative path from script.py location
...
def load_test_data(data_path):
    ...
    # Create label mapping for three classes as specified
    label_mapping = {'non-argumentative': 0, 'premise': 1, 'conclusion': 2}
...
def plot_confusion_matrix(y_true, y_pred, output_path, label_mapping):
    """Plot confusion matrix for three-way classification"""
    ...
    plt.title('Confusion Matrix for Premise vs. Conclusion vs. Non-Argumentative Classification')
    plt.tight_layout()
...
def evaluate_model(model_path, data_path, results_path):
    ...
    labels = sorted(reverse_mapping.keys())
