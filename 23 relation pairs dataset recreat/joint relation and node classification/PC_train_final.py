...
def calculate_node_balanced_class_weights(data_list):
    ...
    total_samples = total_premise + total_conclusion
    
    # Calculate class weights based on the total number of samples
    premise_w = total_samples / (2 * total_premise) if total_premise > 0 else 0
    conclusion_w = total_samples / (2 * total_conclusion) if total_conclusion > 0 else 0

    # Return a weight tensor for just the two classes
    node_class_weights = torch.tensor([premise_w, conclusion_w], dtype=torch.float)
    
    print(f"Node class weights - Premise: {premise_w:.2f}, Conclusion: {conclusion_w:.2f}")
...
def run_joint_cross_validation():
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(all_data)):
        ...
        node_criterion = FocalLoss(weight=node_class_weights.to(device), gamma=2.0)
        ...
        # Training loop
        best_combined_f1 = 0
        node_weight, edge_weight = 0.4, 0.6  
