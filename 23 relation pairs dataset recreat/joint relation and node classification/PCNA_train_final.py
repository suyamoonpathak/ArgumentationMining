...
def calculate_node_balanced_class_weights(data_list):
    """Calculate class weights for node type imbalance correction"""
    total_premise = sum((data.y == 0).sum().item() for data in data_list)
    total_conclusion = sum((data.y == 1).sum().item() for data in data_list)
    total_non_argumentative = sum((data.y == 2).sum().item() for data in data_list)

    total_samples = total_premise + total_conclusion + total_non_argumentative
    
    # Calculate class weights based on the total number of samples
    premise_w = total_samples / (3 * total_premise) if total_premise > 0 else 0
    conclusion_w = total_samples / (3 * total_conclusion) if total_conclusion > 0 else 0
    non_arg_w = total_samples / (3 * total_non_argumentative) if total_non_argumentative > 0 else 0
    
    node_class_weights = torch.tensor([premise_w, conclusion_w, non_arg_w], dtype=torch.float)
    
    print(f"Node class weights - Premise: {premise_w:.2f}, Conclusion: {conclusion_w:.2f}, Non-Argumentative: {non_arg_w:.2f}")

    return node_class_weights


def calculate_edge_balanced_class_weights(data_list):
...
def save_joint_results(node_true, node_pred, edge_true, edge_pred, out_dir):
    ...
    node_report = classification_report(
        node_true, node_pred,
        target_names=["Premise", "Conclusion", "Non-Argumentative"],
        labels=[0,1,2],
        digits=4
    )
...
def plot_node_confusion_matrix(y_true, y_pred, output_path):
    cm = confusion_matrix(y_true, y_pred, labels=[0,1,2])
    cell_labels = np.array([
        [f"Prem→Prem\n{cm[0,0]}", f"Prem→Conc\n{cm[0,1]}", f"Prem→Non-Arg\n{cm[0,2]}"],
        [f"Conc→Prem\n{cm[1,0]}", f"Conc→Conc\n{cm[1,1]}", f"Conc→Non-Arg\n{cm[1,2]}"],
        [f"Non-Arg→Prem\n{cm[2,0]}", f"Non-Arg→Conc\n{cm[2,1]}", f"Non-Arg→Non-Arg\n{cm[2,2]}"]
    ])
...
class JointPredictionLegalRGCN(torch.nn.Module):
    def __init__(self, in_channels=771, hidden_channels=64, num_relations=3, dropout=0.5):
        ...
        # Node classification head (premise vs conclusion vs non-argumentative)
        self.node_classifier = torch.nn.Sequential(
            torch.nn.Linear(hidden_channels, hidden_channels//2),
            torch.nn.ReLU(),
            torch.nn.Dropout(dropout),
            torch.nn.Linear(hidden_channels//2, 3)  # 2 classes: premise, conclusion, non-argumentative
        )
...
def validate_joint(model, val_data, node_criterion, edge_criterion, node_weight=1.0, edge_weight=1.0):
    ...
    node_precision, node_recall, node_f1, _ = precision_recall_fscore_support(
        all_node_true, all_node_preds, average='macro', labels=[0,1,2], zero_division=0
    )
...
def run_joint_cross_validation():
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(all_data)):
        ...
        # Loss functions
        node_class_weights = calculate_node_balanced_class_weights(train_data)
        node_criterion =  FocalLoss(weight=node_class_weights.to(device), gamma=2.0) # For node classification
        edge_class_weights = calculate_edge_balanced_class_weights(train_data)
