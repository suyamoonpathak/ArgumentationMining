...
def get_relation_prediction(source_text, target_text):
    ...
    try:
        prompt = f"""
You are an expert in legal argument analysis. Read the SOURCE and TARGET arguments below and decide if the SOURCE supports the TARGET, attacks the TARGET, or has no relation to it.

This is important. Your answer will help judges in serious cases. A wrong answer could cause mistakes in how arguments are understood.
...
- Only respond with one lowercase word:
    support
    attack
    no-relation
